...
- Домен: управление пользователями (Судя по описанию сервиса мониторинг и управление домом и устройствами требуют авторизации, поэтому авторизация и управление пользователями в отдельном домене)
...
- Синхронное получение данных о температуре от датчика во время запроса инфо о датчике может привести к недоступности приложение при недоступности датчика. Для умного дома просмотр температуры в реальном времени не должно ялвляться критичным, можно показать и устаревшую на 1-5 минут температуру, получив взамен бо&#x301;льшую доступность сервиса
...
![Текст ссылки](schemas/diagrams/context/smarthouse_context.png)
