...
- усложнено масштабирование:
	на чаcть с регистрацией пользователей нагрузка будет явно очень низкая (не часто заводится новый пользователь). На часть с просмотром температуры запросов будет явно больше (например пользователь захочет смотреть каждую минуту на текущую температуру). Часть с управлением, вероятно, будет иметь больше запросов, чем на регистрацию новых пользователей, но меньше чем на просмотр температуры. При увеличении количества пользователей есть смысл выделять разное количество ресурсов на каждую часть приложения.
...
Для общения между фронтенд приложением и микросервисами будет использоваться REST API. Он хорошо стандартизирован, прост, поддерживается множеством фреймворков. Для реализации описанного решения - его будет достаточно.
...
- убрать зависимость сервисов консьюмеров от сбоев сервиса владеющего данными о девайсах</br>
...
- общение между микросервисами через брокер - описано в целевых схемах и в задании 4. Брокер позволит проще добавить следующий запланированный сервис house_management
