...
Телеметрия - опрашивает temperature-api и сохраняет историю, позволяет просматривать историю и последнее значение. Поллит монолит через update_sensor_value чтоб там появилась инфа

Управление устройствами - подключает новый тип устройств (например система отопления) не только сенсоры отопления. можно посматреть все сенсоры системы отопления. Нужно добывать все сенсоры созданные через легаси апи и подвязывать к текущей системе отопления

В монолите сейчас поключаются только сенсоры отопления, но не система отопления (!)
